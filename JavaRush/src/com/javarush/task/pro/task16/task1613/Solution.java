...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        int dateValue = dateTime.getDayOfWeek().getValue();

        if (FRIDAY == dateValue && dateTime.getHour() >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR) {
            return true;
        } else if (dateValue == SATURDAY) {
            return true;
        } else if (dateValue == SUNDAY && dateTime.getHour() < WEEKEND_END_SUNDAY_CUT_OFF_HOUR) {
            return true;
        } else return false;
    }
}
